...
for i, sin2th23 in tqdm(enumerate(S23)):
    for j, delta in enumerate(dcp):
        for k, mass in enumerate(dm23):
            ...
            #val_e_e_NH = P_e_e_NH[i, j, k]
            #val_mu_mu_NH = P_mu_mu_NH[i, j, k]
            #val_mu_e_NH = P_mu_e_NH[i, j, k]

            #val_ebar_ebar_NH = P_ebar_ebar_NH[i, j, k]
            #val_mubar_mubar_NH = P_mubar_mubar_NH[i, j, k]
            #val_mubar_ebar_NH = P_mubar_ebar_NH[i, j, k]
            val_e_NH = P_e_NH[i, j, k]
            val_mu_NH = P_mu_NH[i, j, k]

            # these are indices of the smallest difference in the e-like channel
            abs_diff = np.abs(P_e_IH - val_e_NH)
            #print(abs_diff)
            alpha, beta, gamma = np.unravel_index(np.argmin(abs_diff), abs_diff.shape)
            #print(alpha, beta, gamma)
            diff_e = np.abs(P_e_IH[alpha, beta, gamma] - val_e_NH)
            diff_mu = np.abs(P_mu_IH[alpha, beta, gamma] - val_mu_NH)
            ...
            diffs[i, j, k] = diff_e + diff_mu #diff_mu_e_NH + diff_mubar_ebar_NH# diff_e_e_NH + diff_mu_mu_NH + diff_mu_e_NH + diff_ebar_ebar_NH + diff_mubar_mubar_NH

np.save('fullRangeMinimumDegeneracies', diffs)
...
makePlots(diffs,r'Minimum degeneracy in probabilities between NH and IH - All channels', '../images/degeneracies_map_50_bins_constrained_th23.png', colormap='inferno')
